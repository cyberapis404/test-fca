...
function listenMqtt(defaultFuncs, api, ctx, globalCallback) {
  ...
  global.mqttClient.on('connect', () => {
    if (!global.Fca.Data.Setup || global.Fca.Data.Setup === undefined) {
      if (global.Fca.Require.Sagor.RestartMQTT_Minutes !== 0 && global.Fca.Data.StopListening !== true) {
        ...
        setTimeout(() => {
          ...
          getSeqID();
        }, Number(global.Fca.Require.Sagor.RestartMQTT_Minutes) * 60 * 1000);
      }
    }
    ...
    if (process.env.OnStatus === undefined) {
      ...
      if (Number(global.Fca.Require.Sagor.AutoRestartMinutes) === 0) {
        // something
      } else if (Number(global.Fca.Require.Sagor.AutoRestartMinutes) < 10) {
        log.warn('AutoRestartMinutes', 'The number of minutes to automatically restart must be more than 10 minutes');
      } else if (Number(global.Fca.Require.Sagor.AutoRestartMinutes) < 0) {
        log.warn('AutoRestartMinutes', 'Invalid auto-restart minutes!');
      } else {
        global.Fca.Require.logger.Normal(global.Fca.getText(global.Fca.Require.Language.Src.AutoRestart, global.Fca.Require.Sagor.AutoRestartMinutes));
        global.Fca.Require.logger.Normal(`Auto Restart MQTT Client After: ${global.Fca.Require.Sagor.RestartMQTT_Minutes} Minutes`);
        setTimeout(() => {
          global.Fca.Require.logger.Normal(global.Fca.Require.Language.Src.OnRestart);
          process.exit(1);
        }, Number(global.Fca.Require.Sagor.AutoRestartMinutes) * 60000);
      }
      ...
      const SettingMemoryManager = {
        ...
        interval: 300 * 1000,
        logLevel: 'warn',
        logFile: path.join(process.cwd(), 'Sagor_Database' ,'memory.log'),
        smartReleaseEnabled: true,
        allowLog: (global.Fca.Require.Sagor.AntiStuckAndMemoryLeak.LogFile.Use || false)
      };
      ...
      if (global.Fca.Require.Sagor.AntiStuckAndMemoryLeak.AutoRestart.Use) {
...
if (global.Fca.Require.Sagor.AntiGetInfo.AntiGetThreadInfo) {
    setInterval(() => {
        try {
